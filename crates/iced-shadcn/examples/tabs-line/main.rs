...
impl Example {
    ...
    fn view(&self) -> Element<'_, Message> {
        ...
        let list = tabs_list(
            vec![
                tabs_trigger("overview", "Overview"),
                tabs_trigger("analytics", "Analytics"),
                tabs_trigger("reports", "Reports"),
            ],
            active,
            Some(Message::TabChanged),
            TabsRootProps::new(),
            TabsListProps::new().variant(TabsListVariant::Line),
            theme,
        );

        let content = tabs_contents(
            vec![
                tabs_content("overview", text("Overview content").size(14)),
                tabs_content("analytics", text("Analytics content").size(14)),
                tabs_content("reports", text("Reports content").size(14)),
            ],
            active,
        );
