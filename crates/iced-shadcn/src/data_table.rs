...
use crate::pagination::{
    PaginationProps, pagination, pagination_ellipsis, pagination_link,
...
impl<'a, T, Message> DataTableProps<'a, T, Message> {
    pub fn new(id_source: Id, columns: Vec<DataTableColumn<'a, T, Message>>, data: &'a [T]) -> Self {
        Self {
            id_source,
            columns,
            data,
    ...
    F: Fn(DataTableAction) -> Message + 'a,
{
    ...
    let filtered = filter_rows(props.data, &props.columns, &state.filter, props.filter_fn.as_deref());
    ...
    let mut controls = row![filter_input]
        .spacing(12)
        ...
        |ctx| {
            ...
            if props.enable_selection {
                ...
                let on_toggle = on_action.as_ref().map(|f| {
                    let f = Rc::clone(f);
                    move |next| f(DataTableAction::ToggleAll(matches!(next, CheckboxState::Checked)))
                });
                header_cells.push(
                    table_head(
                        ctx,
            ...
            for (index, column) in props.columns.iter().enumerate() {
                ...
                let header_content = row![
                    text(column.header.clone()).size(12),
                    indicator_element
                ]
                .spacing(4)
                .align_y(Alignment::Center);
            ...
            if page_rows.is_empty() {
                ...
                let empty_text = text("No results.")
                    .size(12)
